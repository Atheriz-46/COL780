...
def model(args):
    ...
    backSub = [cv.createBackgroundSubtractorKNN()]*3#0.3007
    ...
    for file in sorted(os.listdir(args.inp_path)):
        if file[-4:] in ['.png','.jpg']:
            ...
            for i,fr in enumerate(frame):
                fgMask+=[backSub[i].apply(frame[i])]

                if frame_no>=start and frame_no<=end:
                    fgMask[i]=remove_noise(fgMask[i],3)
                
            if frame_no>=start and frame_no<=end:
                
                fgMask=np.array(fgMask).transpose((1,2,0))
                fgMask=np.where(np.any(fgMask,axis=2),255,0)
                # fgMask=cv.cvtColor(fgMask, cv.COLOR_BGR2GRAY)
                # _,fgMask = cv.threshold(fgMask, 126, 255, cv.THRESH_BINARY)
